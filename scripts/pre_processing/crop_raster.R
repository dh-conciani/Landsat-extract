...
reference <- shapefile ("H:/shps/limites/buffer_23n.shp")

# Get reference projection
ref_proj <- crs(reference)

# Get the list of rasters
raster_list <- list.files(path = 'H:/machine_learning/imgs/', pattern = '.tif$', full.names = T)
...
for (i in 1:list_count) {
  ...
  #perform crop
  croped_raster <- crop (r, reference)
  #write each reprojected raster to a new file 
  writeRaster(croped_raster, paste0 ('H:/machine_learning/img_croped/',list_names[i],'.tif'), overwrite=TRUE)
}
