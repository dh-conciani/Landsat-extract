...
raster_loc <- list.files(path = 'H:/machine_learning/img_reprojected/ETM/', pattern = '.tif$', full.names = T)
...
ETM_data_entries <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  ## build data.frame
  ETM_data_entries <- rbind (ETM_data_entries, temp)
}
...
scene_points <- nrow(ETM_data_entries) /nlevels(ETM_data_entries$bname)

## Create empty data.frame for non-burned data
ETM_non_burned <- data.frame (NULL)
...
for (i in 1:list_count) {
  ...
  temp <- data.frame (bname, date, sensor, temp)
  ## build data.frame
  ETM_non_burned <- rbind (ETM_non_burned, temp)
}
